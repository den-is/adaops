...
from adaops.init_helpers import get_era_arg, get_truthy_value, net_arg
...
# Depends on the official env var `CARDANO_NODE_NETWORK_ID` and accepts its values
NET_ARG = net_arg(str(config.get("CARDANO_NODE_NETWORK_ID", "mainnet")))

CARDANO_CLI_LEGACY_COMMANDS = get_truthy_value(config.get("CARDANO_CLI_LEGACY_COMMANDS", True))

# LEGACY_ERA_ARG returns --shelley-era or --byron-era
LEGACY_ERA_ARG = get_era_arg(
